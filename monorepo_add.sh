...
# It will merge 0.3 branches of the monorepo and all remotes together while keeping all current branches in monorepo intact
...
for PARAM in $@; do
    ...
    echo "Building branch '0.3' of the remote '$REMOTE'"
    git checkout --detach $REMOTE/0.3
...
for REF in $MERGE_REFS; do
    # Add all files from all 0.3 branches into index
